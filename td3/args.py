import argparse


def get_td3_args_train():
    parser = argparse.ArgumentParser()
    parser.add_argument("--seed", default=0, type=int)  # Sets Gym, PyTorch and Numpy seeds
    parser.add_argument("--start_timesteps", default=1e4, type=int)  # How many time steps purely random policy is run for
    parser.add_argument("--eval_freq", default=5e3, type=float)  # How often (time steps) we evaluate
    parser.add_argument("--max_timesteps", default=2e5, type=float)  # Max time steps to run environment for
    parser.add_argument("--save_models", action="store_true", default=True)  # Whether or not models are saved
    parser.add_argument("--expl_noise", default=0.1, type=float)  # Std of Gaussian exploration noise
    parser.add_argument("--batch_size", default=32, type=int)  # Batch size for both actor and critic
    parser.add_argument("--discount", default=0.99, type=float)  # Discount factor
    parser.add_argument("--tau", default=0.005, type=float)  # Target network update rate
    parser.add_argument("--policy_noise", default=0.2, type=float)  # Noise added to target policy during critic update
    parser.add_argument("--noise_clip", default=0.5, type=float)  # Range to clip target policy noise
    parser.add_argument("--policy_freq", default=2, type=int)  # Frequency of delayed policy updates
    parser.add_argument("--env_timesteps", default=500, type=int)  # Frequency of delayed policy updates
    parser.add_argument("--replay_buffer_max_size", default=10000, type=int)  # Maximum number of steps to keep in the replay buffer
    parser.add_argument("--exp_label", default="debug", type=str)
    parser.add_argument("--add_bn", default=True, action = 'store_false')
    parser.add_argument("--grad_norm", default=False, action = 'store_false')
    parser.add_argument("--fc_hid_size", default=256, type=int)
    parser.add_argument("--spe_init", default=True, action = "store_true")
    parser.add_argument("--recurrent", default=False, action = "store_true")
    parser.add_argument("--frame_stack_k", default=3, type=int)
    parser.add_argument("--priority_replay", default=False, action="store_true")
    parser.add_argument('--priority-exponent', type=float, default=0.5, metavar='omega', help='Prioritised experience replay exponent (originally denoted alpha)')
    parser.add_argument('--priority-weight', type=float, default=0.4, metavar='beta', help='Initial prioritised experience replay importance sampling weight')
    parser.add_argument("--critic_lr", default=1e-3, type=float)
    parser.add_argument("--actor_lr", default=1e-4, type=float)
    parser.add_argument("--net_type", default="cnn", type=str)
    parser.add_argument("--load_model", default=False, type = bool)

    return parser.parse_args()


def get_td3_args_test():
    parser = argparse.ArgumentParser()
    parser.add_argument("--seed", default=0, type=int)  # Sets Gym, PyTorch and Numpy seeds
    parser.add_argument("--experiment", default=2, type=int)
    parser.add_argument("--exp_label", default="debug", type=str)
    parser.add_argument("--model_label", default="final", type=str)
    parser.add_argument("--map_name", default="loop_empty", type=str)
    parser.add_argument("--add_bn", default=True, action = 'store_false') 
    parser.add_argument("--fc_hid_size", default=512, type=int)
    parser.add_argument("--spe_init", default=False, action = "store_true")
    parser.add_argument("--frame_stack_k", default=3, type=int)
    parser.add_argument("--net_type", default="cnn", type=str)
    parser.add_argument("--critic_lr", default=1e-3, type=float)
    parser.add_argument("--actor_lr", default=1e-4, type=float)
    parser.add_argument("--noisy_linear", default=False, type = bool)
    parser.add_argument("--epi", default=1, type=int)
    parser.add_argument("--savefolder", default="lstmg", type=str)
    parser.add_argument("--noise", default=0, type=float)
    parser.add_argument("--gpu", default=0,type=str)


    return parser.parse_args()
